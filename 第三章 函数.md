...
mixArgs("a")
```
结果为：1、true、false、false、false。（注意如果second赋值为undefined，不是'undefined'，则second===arguments[1]为true，因为始终绕不过JS的定义）
### 参数默认值表达式
...
```
### 创建立即调用函数表达式
使用传统函数时，(function(){函数体})()与(function(){函数体}())这两种方式都是可行的，且匿名函数可以使用
```
var foo=function(){}()
```
但是若使用箭头函数，只有(()=>{函数体})()
...
```
此处的this是对事件目标对象（也就是document）的一个引用，而没有绑定到PageHandler上。可以明确使用bind()方法将函数的this值绑定到PageHandler上。
```
init:function(){
	document.addEventListener("click".(function(event){
			this.doSomething(event.type);
